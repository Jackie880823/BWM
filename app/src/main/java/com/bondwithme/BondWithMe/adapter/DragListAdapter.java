...
//        }
    ...
    public void exchange(int startPosition, int endPosition) {
        ...
        if (startPosition < endPosition) {
            mDataList.add(endPosition + 1, (LocalStickerInfo) startObject);
            mDataList.remove(startPosition);
        } else {
            mDataList.add(endPosition, (LocalStickerInfo) startObject);
    ...
    public void exchangeCopy(int startPosition, int endPosition) {
        ...
        if (startPosition < endPosition) {// 向下移动
            mCopyList.add(endPosition + 1, (LocalStickerInfo) startObject);
            mCopyList.remove(startPosition);
        } else {// 向上拖动或者不动
            mCopyList.add(endPosition, (LocalStickerInfo) startObject);
    ...
    public void addDragItem(int start, Object obj) {
        ...
        mDataList.add(start, (LocalStickerInfo) obj);// 添加删除项
    }

    private ArrayList<LocalStickerInfo> mCopyList = new ArrayList<LocalStickerInfo>();

    public void copyList() {
        mCopyList.clear();
        for (LocalStickerInfo str : mDataList) {
            mCopyList.add(str);
        }
    }
    ...
    public void pastList() {
        mDataList.clear();
        for (LocalStickerInfo str : mCopyList) {
            mDataList.add(str);
        }
    }
