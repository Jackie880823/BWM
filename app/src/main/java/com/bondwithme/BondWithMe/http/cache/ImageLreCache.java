package com.bondwithme.BondWithMe.http.cache;
...
public class ImageLreCache extends LruCache<String, Bitmap> implements ImageLoader.ImageCache {
    ...
    public ImageLreCache(int maxSize, String diskCacheFodler, int diskCacheSize) {
        ...
        try {
            mDiskLruCache = DiskLruCache.open(getDiskCacheDir(App.getContextInstance(), CACHE_FOLDER_NAME),
                    getAppVersion(App.getContextInstance()), 1, DISK_CACHE_SIZE);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
