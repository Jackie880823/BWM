 ...
 * {@link com.bondwithme.BondWithMe.ui.TabWordFragment.OnFragmentInteractionListener} interface
 * to handle interaction events.
 * Use the {@link com.bondwithme.BondWithMe.ui.TabWordFragment#newInstance} factory method to
...
public class TabWordFragment extends BaseFragment<WallNewActivity> {
    ...
    public void changeAtDesc(List<UserEntity> members, List<GroupEntity> groups, boolean checkVisible) {
        ...
        if(members != null && mMembers.size() > 0) {
            memberText = String.format(getParentActivity().getString(R.string.text_diary_content_at_member_desc), mMembers.size());
        ...
        if(groups != null && mGroups.size() > 0) {
            groupText = String.format(getParentActivity().getString(R.string.text_diary_content_at_group_desc), mGroups.size());
